...
class traj:
    ...
    def circle(self, t, init_pos=None, r=None, period=None, circle_duration=None):
        ...
        if (np.all(init_pos is not None)) and (r is not None) and (period is not None) and (circle_duration is not None):
    ...
    def line_quintic_traj(self, t, map=None, path=None):
        ...
        # map is a class
        # path is a 2D array
        if (np.any(map is not None)) and (np.any(path is not None)):
    ...
    def min_snap_traj_generator(self, t_current, path=None, options=None):
        ...
        if (np.any(path is not None)) and (np.any(options is not None)):
    ...
    def step_des(self, t, des_pose=None):
        if (np.any(des_pose is not None)):
