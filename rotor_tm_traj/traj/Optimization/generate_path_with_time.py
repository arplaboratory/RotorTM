...
def generate_path_with_time(path=None, traj_constant=None, T_seg=None, t_start=None):
    if (np.any(path is not None)) and (np.any(traj_constant is not None)) and (np.any(T_seg is not None)) and (np.any(t_start is not None)):
        time = t_start
    ...
    elif (np.any(path is not None)) and (np.any(traj_constant is not None)) and (np.any(T_seg is not None)):
        time = 0.0
        time_list = np.array([[time]])
