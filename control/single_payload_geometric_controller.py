...
def single_payload_geometric_controller(ql = None,t = None,qd_params = None,pl_params = None): 
...
# Desired cable direction
    ...
    w_des_dot_ = np.array([[0],[0],[0]])
    
    mu_ = xixiT_ * mu_des_
    e_xi = cross(xi_des_,xi_)
    e_w = w_ + xi_asym_ * xi_asym_ * w_des_
    Force = mu_ - quad_m * l * cross(xi_,qd_params.Kxi * e_xi + qd_params.Kw * e_w + (np.transpose(xi_) * w_des_) * xidot_ + xi_asym_ * xi_asym_ * w_des_dot_)
...
#q_des = (m/F)*(jrk_des - (Z_body_in_world'*jrk_des)*Z_body_in_world)'*X_body_in_world;
    ...
    e_omega = ql.qd_omega - Rot_worldtobody * Rot_des * np.transpose(np.array([p_des,q_des,r_des]))
    # Moment
